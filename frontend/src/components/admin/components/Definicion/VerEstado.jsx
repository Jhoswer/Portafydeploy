...
export default function VerEstado() {
  ...
  const { records, isLoading, error } = useDefinitionRecords("states");
    ...
    <div className="def-ver">
      <div className="def-ver__toolbar">
        <div className="def-ver__search-wrap">
          ...
          <input className="def-ver__search" placeholder="Buscar por nombre, pais o estado..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      <div className="def-ver__table-wrap">
        <table className="def-ver__table">
          <thead><tr><th>ID</th><th>Nombre</th><th>Pais</th><th>Estado</th><th>Creado</th><th>Actualizado</th></tr></thead>
          <tbody>
            {isLoading || error || filtered.length === 0 ? (
              <tr><td colSpan={6} className="def-ver__empty">{isLoading ? "Cargando registros..." : error || `Sin resultados para "${search}"`}</td></tr>
            ) : filtered.map((r) => (
              <tr key={r.id_state_country}>
                <td className="def-ver__id">#{r.id_state_country}</td>
