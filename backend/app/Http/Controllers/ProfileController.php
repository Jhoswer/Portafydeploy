...
class ProfileController extends Controller
{
    ...
    /**
     * Búsqueda para módulo admin (Edición)
     * Busca usuarios por nombre y filtra por rol
     * GET /api/admin/users/search?q=<query>&role=<role>
     */
    public function adminSearchUsers(Request $request): JsonResponse
    {
        try {
            $query = (string) $request->query('q', '');
            $role = (string) $request->query('role', 'todos');

            $results = $this->profileSearchService->searchByNameAndRole($query, $role);
    ...
    public function adminShowProfile(Profile $profile): JsonResponse
    {
        return response()->json($this->profileService->adminShowProfile($profile));
    }

    public function adminUpdateProfile(AdminProfileUpdateRequest $request, Profile $profile): JsonResponse
    {
        try {
            return response()->json([
                'status' => 'success',
                'message' => 'Perfil actualizado correctamente',
                'data' => $this->profileService->adminUpdateProfile($request, $profile),
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
