...
// Formulario inline de creación de Reclutador — usa clases de EdicionModalesTablas.css
...
export default function CreacionReclutadorForm({ onCancel }) {
  ...
  const validate = () => {
    ...
    if (!fields.company.trim())
      next.company = "El nombre de empresa es obligatorio.";
    else if (fields.company.trim().length < 2)
      next.company = "Mínimo 2 caracteres.";
    if (!fields.email)
      next.email = "El correo corporativo es obligatorio.";
      ...
      <div style={{ width: "100%", padding: "28px 32px 16px" }}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="edicion-modal__fields">
            ...
            {/* Nombre de empresa */}
            <InputField label="Nombre de empresa" value={fields.company} onChange={setField("company")}
              placeholder="Acme Inc." icon={Building2} error={errors.company}
              autoComplete="organization" required />

            {/* Email corporativo */}
            <InputField label="Correo corporativo" type="email"
